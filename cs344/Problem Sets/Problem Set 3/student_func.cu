...
#include "utils.h"
    using namespace std;
__global__ void switch_value_per_block(const float *const d_logLuminance, float *const d_switched_logLuminance,
                                       const size_t arr_len);

__global__ void find_min_max_per_block(const float *const d_switched_logLuminance, const size_t arr_len,
                                       float *const d_min_arr, float *const d_max_arr);

__global__ void find_min_max_global(float *const d_min_arr, float *const d_max_arr, const size_t arr_len);

__global__ void compute_histo_bins(const float *const d_logLuminance, const size_t arr_len,
                                   unsigned int *const d_histo_bins, const size_t num_bins, const float min_logLum,
...
void your_histogram_and_prefixsum(const float *const d_logLuminance, unsigned int *const d_cdf, float &min_logLum,
    ...
    /*Here are the steps you need to implement
      ...
      4) Performan exclusive scan (prefix sum) on the histogram to get the cumulative
    ...
    */
    ...
    switch_value_per_block<<<grid_size, block_size, sizeof(float) * block_size.x>>>(d_logLuminance,
                                                                                    d_switched_logLuminance, arr_len);
    ...
    find_min_max_per_block<<<grid_size, block_size, sizeof(float) * block_size.x>>>(d_switched_logLuminance, arr_len,
                                                                                    d_min_arr, d_max_arr);
    checkCudaErrors(cudaFree(d_switched_logLuminance));
    find_min_max_global<<<1, grid_len, sizeof(float) * grid_len * 2>>>(d_min_arr, d_max_arr, grid_len);

    //# Copy switched date back to CPU
    checkCudaErrors(cudaMemcpy(&min_logLum, d_min_arr, sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(&max_logLum, d_max_arr, sizeof(float), cudaMemcpyDeviceToHost));
    ...
    checkCudaErrors(cudaMemset(d_histo_bins, 0,  sizeof(unsigned int) * numBins));
    compute_histo_bins<<<grid_size, block_size>>>(d_logLuminance, arr_len, d_histo_bins, numBins, min_logLum,
                                                  max_logLum);
    cout << "Min MAX " << min_logLum << " : " << max_logLum << endl;
    unsigned int *h_histo_bins = new unsigned int[numBins];
    checkCudaErrors(cudaMemcpy(h_histo_bins, d_histo_bins, sizeof(unsigned int) * numBins, cudaMemcpyDeviceToHost));
    /*float logLum_range = max_logLum - min_logLum;
    float * h_logLuminance = (float *)malloc(sizeof(float)*arr_len);
    checkCudaErrors(cudaMemcpy(h_logLuminance,d_logLuminance,sizeof(float)*arr_len,cudaMemcpyDeviceToHost));
    for(int i = 0;i < arr_len;++ i){
        unsigned int max_index = numBins - 1;
        unsigned int tmp_index = (h_logLuminance[i] - min_logLum) / logLum_range * numBins;
        unsigned int bin_index = max_index < tmp_index ? max_index : tmp_index;
        h_histo_bins[bin_index]++;
    }
    */
...
__global__ void find_min_max_per_block(const float *const d_switched_logLuminance, const size_t arr_len,
    ...
    for (int interval = tmp_arr_len / 4; interval > 0; interval /= 2) {
        ...
        if (s_arr_index >= tmp_arr_len / 2 && s_arr_index < tmp_arr_len / 2 + interval) {
...
__global__ void compute_histo_bins(const float *const d_logLuminance, const size_t arr_len,
    ...
    unsigned int tmp_index = (d_logLuminance[g_index] - min_logLum) / logLum_range * num_bins;
    unsigned int bin_index = max_index < tmp_index ? max_index : tmp_index;
